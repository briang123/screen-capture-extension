...
export const CaptureOverlay: React.FC<CaptureOverlayProps> = ({
  ...
  const [pageDims, setPageDims] = React.useState(getPageDimensions());
  ...
  return (
    <AnimatePresence>
      <motion.div
        ref={overlayRef}
        className="sc-capture-overlay fixed left-0 top-0 z-[9999] bg-black/30 backdrop-blur-[1px]"
        style={{
          position: 'absolute',
          left: 0,
          top: 0,
          ...
          pointerEvents: 'none', // Let the hook handle mouse events
        }}
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
